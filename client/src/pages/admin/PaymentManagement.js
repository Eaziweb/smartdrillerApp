...
const PaymentManagement = () => {
  ...
  const handleRetryPayment = async (paymentId) => {
    ...
    // Check if payment has Flutterwave reference
    if (!payment.flutterwaveRef) {
      showMessage("This payment cannot be retried because it doesn't have a Flutterwave reference", "error")
      return
    }
                  ...
                  filteredPayments.map((payment) => (
                    <tr key={payment._id}>
                      ...
                      <td>
                        ...
                        {payment.status === "pending" && payment.flutterwaveRef && (
                          <button 
                            className={styles.editBtn}
                            onClick={() => handleRetryPayment(payment._id)}
                            disabled={retryingPayment === payment._id}
                        ...
                        {payment.status === "pending" && !payment.flutterwaveRef && (
                          <span 
                            className={styles.noFlutterwaveRef}
                            title="Cannot retry: No Flutterwave reference"
                          >
      ...
      {showPaymentDetails && selectedPayment && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.detailsGrid}>
                ...
                <div className={styles.detailsRow}>
                  <div className={styles.detailsLabel}>Flutterwave Reference:</div>
                  <div className={styles.detailsValue}>
                    {selectedPayment.flutterwaveRef || (
                      <span style={{ color: "var(--error-color)" }}>
                        Not available - Cannot retry this payment
                      </span>
                    )}
                  </div>
                </div>
            ...
            <div className={styles.modalFooter}>
              {selectedPayment.status === "pending" && selectedPayment.flutterwaveRef && (
                <button 
                  className={styles.submitBtn}
                  onClick={() => {
                    setShowPaymentDetails(false)
              ...
              {selectedPayment.status === "pending" && !selectedPayment.flutterwaveRef && (
                <div className={styles.noFlutterwaveRef}>
                  <i className="fas fa-exclamation-triangle"></i> This payment cannot be retried because it doesn't have a Flutterwave reference.
                </div>
              )}
              <button 
                className={styles.cancelBtn}

...
const NoteManagement = () => {
  ...
  const handleCreateCourse = async (e) => {
    ...
    try {
      if (editingItem) {
        await axios.put(`/api/admin/notes/courses/${editingItem._id}`, courseForm, {
          headers: getAuthHeader(),
        })
      } else {
  ...
  const handleCreateNote = async (e) => {
    ...
    try {
      if (editingItem) {
        ...
        await axios.post(`/api/admin/notes/courses/${selectedCourse._id}/notes`, noteForm, {
          headers: getAuthHeader(),
        })
      }
      ...
      setEditingItem(null)
      loadCourses()
      showToast(editingItem ? "Note updated successfully!" : "Note created successfully!")
    } catch (error) {
  ...
  const handleDeleteCourse = async (courseId) => {
    if (!window.confirm("Are you sure you want to delete this course and all its notes?")) return
    try {
                ...
                {course.notes.map((note) => (
                  <div key={note._id} className={styles.noteItem}>
                    ...
                    <div className={styles.noteActions}>
                      <button className={`${styles.btn} ${styles.btnSm} ${styles.btnOutline}`} onClick={() => openEditNote(note)}>
                        <i className="fas fa-edit"></i>
                      </button>
                      <button className={`${styles.btn} ${styles.btnSm} ${styles.btnDanger}`} onClick={() => handleDeleteNote(note._id)}>
                        <i className="fas fa-trash"></i>
                      </button>
                    </div>
                  </div>
            ...
            <form onSubmit={handleCreateNote} className={styles.modalForm}>
              ...
              <div className={styles.formGroup}>
                ...
                <div className={styles.editorToolbar}>
                  ...
                  <button type="button" onClick={() => formatText("h3")} title="Heading 3">
                    H3
                  </button>
                  <button type="button" onClick={() => formatText("ul")} title="Bullet List">
                    <i className="fas fa-list-ul"></i>
                  </button>
                  <button type="button" onClick={() => formatText("ol")} title="Numbered List">

...
router.delete("/:id", adminAuth, async (req, res) => {
  try {
    ...
    const questions = await CompetitionQuestion.find({ competitionId: competition._id });
    
    // Delete all images from Cloudinary
    for (const question of questions) {
      if (question.imagePublicId) {
        await deleteCloudinaryImage(question.imagePublicId);
      }
    }
...
router.delete("/:id/courses/:courseCode", adminAuth, async (req, res) => {
  try {
    ...
    // Get all questions for this course to delete images
...
router.post("/:id/questions", adminAuth, upload.single("image"), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      questionData.imagePublicId = req.file.filename;
    }
...
router.put("/:id/questions/:questionId", adminAuth, upload.single("image"), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      if (questionDoc.imagePublicId) {
        await deleteCloudinaryImage(questionDoc.imagePublicId);
      }
      
      questionDoc.image = req.file.path;
      questionDoc.imagePublicId = req.file.filename;
    }

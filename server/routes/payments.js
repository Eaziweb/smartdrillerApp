...
router.post("/retry-verification/:paymentId", auth, async (req, res) => {
  try {
    ...
    // Check if payment has a Flutterwave reference
    if (!payment.flutterwaveRef) {
      return res.status(400).json({
        success: false,
        message: "This payment cannot be retried because it doesn't have a Flutterwave reference",
      });
    }
    ...
    // Use Flutterwave reference to verify the transaction
    const response = await axios.get(
      `https://api.flutterwave.com/v3/transactions/${payment.flutterwaveRef}/verify`,
      {
        headers: {
          Authorization: `Bearer ${process.env.FLUTTERWAVE_SECRET_KEY}`,
        },
